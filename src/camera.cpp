...
std::string pressed_key{""};  // Later maybe change to received command
 ...
 * 
 * @return true If the camera setup is successful.
 * @return false If the camera setup fails.
 */
bool Camera::camera_setup()
{
    ...
    std::jthread listen_thread(listen_command, std::ref(pressed_key));
 ...
 * @return true If the camera start is successful.
 * @return false If the camera start fails.
 */
bool Camera::camera_start(const std::string& camera_name, int camera_ID)
{
    while (true)
    {
        ...
        pressed_key.clear();
        lock.unlock();  // Lets other parts access to shared data
        ...
        }  // End while(pressed_key != camera_name.substr(0, 1))
